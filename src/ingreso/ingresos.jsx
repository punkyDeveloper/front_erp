...
const TIPOS = [
  "Servicio", "Venta de producto", "Consulta", "Membresía",
  "Comisión", "Propina", "Devolución recibida", "Otro"
];
...
.ing-scroll { flex:1; overflow-y:auto; overflow-x:hidden; min-width:0; }
...
.ing-head h1 { font-size:1.4rem; font-weight:800; letter-spacing:-0.03em; margin:0; color:#fff; display:flex; align-items:center; gap:10px; }
.ing-head .sub { font-size:0.82rem; color:#4e4f60; margin-top:3px; }

.btn-new {
  padding: 10px 22px; border: none; border-radius: 10px; background: #10b981;
...
.ing-body { padding: 0 28px 36px; }

.ing-kpis { display:grid; grid-template-columns:repeat(3,1fr); gap:12px; margin-bottom:18px; }
.ing-kpi { background:#1a1b27; border-radius:14px; padding:18px 20px; border:1px solid #22232e; }
.ing-kpi .kl { font-size:0.67rem; text-transform:uppercase; letter-spacing:0.07em; color:#4e4f60; font-weight:700; margin-bottom:4px; }
.ing-kpi .kv { font-size:1.35rem; font-weight:800; color:#fff; letter-spacing:-0.02em; }
.ing-kpi .kv.green { color:#10b981; }

.ing-toolbar { display:flex; gap:10px; margin-bottom:14px; flex-wrap:wrap; align-items:center; }
.ing-search {
...
.ing-search:focus { border-color:#10b981; }
.ing-search::placeholder { color:#3e3f50; }
...
.ing-card { background:#1a1b27; border-radius:14px; border:1px solid #22232e; overflow:hidden; margin-bottom:14px; }
...
.tbl tbody tr:hover { background:rgba(16,185,129,0.03); }

.tipo-badge { display:inline-block; padding:3px 10px; border-radius:6px; font-size:0.72rem; font-weight:700; background:rgba(16,185,129,0.1); color:#10b981; }
.ref { font-family:monospace; font-size:0.78rem; color:#10b981; font-weight:600; }
...
.pag-btn:hover:not(:disabled) { border-color:#10b981; color:#fff; }
.pag-btn:disabled { opacity:0.3; cursor:default; }
.pag-btn.on { background:#10b981; color:#fff; border-color:#10b981; }
...
.field input:focus, .field select:focus, .field textarea:focus { border-color:#10b981; }
...
.btn-save { padding:10px 24px; border:none; border-radius:10px; background:#10b981; color:#fff; font-size:0.85rem; font-weight:700; font-family:inherit; cursor:pointer; }
...
@media(max-width:768px) {
  .ing-head { padding:18px 14px; }
  .ing-body { padding:0 14px 28px; }
  .ing-kpis { grid-template-columns:1fr; }
  .ing-toolbar { flex-direction:column; }
  .ing-search { min-width:100%; }
...
/* ═══════════════════════════════════════════════════════════
   COMPONENT
   ═══════════════════════════════════════════════════════════ */
const EMPTY_FORM = { fecha: hoy(), valor: "", tipo: "Servicio", descripcion: "" };
...
export default function Ingresos() {
  ...
  // ── GET solo ingresos de la empresa (filtrado por JWT + query param) ──
  const fetchData = useCallback(async () => {
    try {
      const res = await fetch(`${API_URL}?tipo_movimiento=ingreso`, {
        headers: getHeaders(),
      });
  ...
  const handleSave = async () => {
    ...
    try {
      const body = {
        tipo_movimiento: "ingreso",  // ← siempre ingreso
        fecha: form.fecha,
      ...
      setAlert({ type: "ok", msg: editing ? "Ingreso actualizado" : "Ingreso registrado" });
      closeModal();
      fetchData();
    } catch (error) {
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      setDeleteTarget(null);
      setAlert({ type: "ok", msg: "Ingreso eliminado" });
      fetchData();
    } catch {
    ...
    <div style={{ display: "flex", height: "100vh" }}>
      ...
      <div className="ing ing-scroll">
        ...
        <div className="ing-head">
          <div>
            <h1>💰 Ingresos</h1>
            <div className="sub">Registro de ingresos del negocio</div>
          </div>
          {puedeCrear && (
            <button className="btn-new" onClick={openNew}>+ Nuevo ingreso</button>
          )}
        </div>
        ...
        <div className="ing-body">
          ...
          <div className="ing-kpis">
            <div className="ing-kpi"><div className="kl">Total registros</div><div className="kv">{data.length}</div></div>
            <div className="ing-kpi"><div className="kl">Ingresos del mes</div><div className="kv green">{fCOP(totalMes)}</div></div>
            <div className="ing-kpi"><div className="kl">Ingresos totales</div><div className="kv green">{fCOP(totalGeneral)}</div></div>
          </div>

          {/* ── Toolbar ── */}
          <div className="ing-toolbar">
            <input className="ing-search" placeholder="Buscar por descripción, referencia, tipo, creador..." value={search} onChange={e => setSearch(e.target.value)} />
            <select className="ing-filter" value={filterTipo} onChange={e => setFilterTipo(e.target.value)}>
              ...
              <table className="tbl">
                ...
                <tbody>
                  {paginated.length > 0 ? paginated.map(d => (
                    <tr key={d._id}>
                      <td><span className="ref">{d.referencia || "—"}</span></td>
                      <td style={{ whiteSpace: "nowrap" }}>{fDate(d.fecha)}</td>
                      <td><span className="tipo-badge">{d.tipo}</span></td>
                    ...
                    <tr>
                      <td colSpan={colCount}>
                        <div className="empty">
                          ...
                          <h3>Sin registros</h3>
                          <p>No hay ingresos que coincidan</p>
                        </div>
                      </td>
                    </tr>
                  )}
      ...
      {/* ── Modal Crear/Editar ── FUERA del scroll ── */}
      {showModal && (
          ...
          <div className="md-box" onClick={e => e.stopPropagation()}>
            ...
            <div className="md-body">
              ...
              <div className="field">
                ...
                <textarea placeholder="Detalle del ingreso..." value={form.descripcion} onChange={e => upd("descripcion", e.target.value)} />
              </div>
            </div>
            <div className="md-foot">
              <button className="btn-cancel" onClick={closeModal}>Cancelar</button>
              <button className="btn-save" onClick={handleSave} disabled={saving || !form.valor || !form.fecha || !form.descripcion}>
      ...
      {/* ── Modal Confirmar Delete ── FUERA del scroll ── */}
      {deleteTarget && (
          ...
          <div className="md-box" style={{ maxWidth: 400 }} onClick={e => e.stopPropagation()}>
            <div className="confirm-body">
              ...
              <p>
                ...
                <br />
                <span style={{ color: "#10b981", fontWeight: 700 }}>{fCOP(deleteTarget.valor)}</span>
              </p>
